...
def convertToPipeDirectionBoard(pipeDirectionNumber):
...
class PipeSolver:
    ...
    def isValidMove(self, currentDirectionBoard, pipePos, operation):
        x = pipePos[0]
        y = pipePos[1]
        directions = self.pipeDirectionsHelper.get_directions_pipe_point_to(pipePos, operation)
        directions_above = self.pipeDirectionsHelper.get_directions_pipe_point_to((x - 1, y), currentDirectionBoard[x - 1][y])
        directions_left = self.pipeDirectionsHelper.get_directions_pipe_point_to((x, y - 1), currentDirectionBoard[x][y - 1])

        matchLeft = xnor((Direction.LEFT in directions), (Direction.RIGHT in directions_left))
        matchTop = xnor((Direction.UP in directions), (Direction.DOWN in directions_above))
    ...
    def BFS(self):
        ...
        openList = [] # queue
        initialNumOfPipeMismatches = self.getNumOfPipeMismatches_init(self.pipeDirectionBoard)
        firstItem = queueItem(self.pipeDirectionBoard, (0,0), [], initialNumOfPipeMismatches)
...
pipeDirectionBoard = convertToPipeDirectionBoard(pipeDirectionNumber)
